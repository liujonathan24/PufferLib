    ...
    for (i = 2; i <= WIDTH; i++)
        for (j = 1; j < HEIGHT; j++) {
            if (levl[i][j].typ == fg_typ && levl[i][j].roomno == NO_ROOM) {
                ...
                min_ry = max_ry = j;
                n_loc_filled = 0;
                flood_fill_rm(i, j, current_nle_ctx->nroom + ROOMOFFSET, FALSE, FALSE);
                if (n_loc_filled > 3) {
                    add_room(min_rx, min_ry, max_rx, max_ry, FALSE, OROOM,
                             TRUE);
                    rooms[current_nle_ctx->nroom - 1].irregular = TRUE;
                    if (current_nle_ctx->nroom >= (MAXNROFROOMS * 2))
                        goto joinm;
                } else {
                    /*
                    ...
                    for (sx = min_rx; sx <= max_rx; sx++)
                        for (sy = min_ry; sy <= max_ry; sy++)
                            if ((int) levl[sx][sy].roomno
                                == current_nle_ctx->nroom + ROOMOFFSET) {
...
joinm:
     ...
     */
    for (croom = &rooms[0], croom2 = croom + 1; croom2 < &rooms[current_nle_ctx->nroom];) {
...
int lx, ly, hx, hy;
{
    ...
    for (i = current_nle_ctx->nroom - 1; i >= 0; --i) {
        croom = &rooms[i];
...
/*
 * Remove roomno from the rooms array, decrementing current_nle_ctx->nroom.  Also updates
...
unsigned roomno;
{
    ...
    struct mkroom *maxroom = &rooms[--current_nle_ctx->nroom];
    int i, j;
    unsigned oroomno;
    ...
    if (croom != maxroom) {
        ...
        oroomno = current_nle_ctx->nroom + ROOMOFFSET;
        roomno += ROOMOFFSET;
