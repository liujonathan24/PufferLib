...
if [ "$PLATFORM" = "Linux" ]; then
    ...
    # OMP_LIB defaults to Intel OpenMP (-liomp5, ships as libiomp5.so in
    # Intel oneAPI). Override with e.g. OMP_LIB=-lgomp for GCC OpenMP or
    # OMP_LIB=-lomp5 for the older non-Intel naming.
    OMP_LIB=${OMP_LIB:--liomp5}
    SANITIZE_FLAGS=(-fsanitize=address,undefined,bounds,pointer-overflow,leak -fno-omit-frame-pointer)
    STANDALONE_LDFLAGS=(-lGL)
    SHARED_LDFLAGS=(-Bsymbolic-functions)
else
    RAYLIB_NAME='raylib-5.5_macos'
    OMP_LIB=${OMP_LIB:--lomp}
    SANITIZE_FLAGS=()
...
if [ "$MODE" = "local" ] || [ "$MODE" = "fast" ]; then
    ...
    # Honor EXTRA_CFLAGS env var (e.g. EXTRA_CFLAGS="-DNETHACK_PROFILE=1").
    if [ -n "$EXTRA_CFLAGS" ]; then
        FLAGS=($EXTRA_CFLAGS "${FLAGS[@]}")
    fi
    echo "Compiling $ENV..."
